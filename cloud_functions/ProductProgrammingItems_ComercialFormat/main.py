        ...
        for product in products:
            ...
            for programming in programming_list:
                ...
                for items in items_list:
                    CommercialFormat_dict = items.get("format")   
                    row = {
                        "ITEMS_ID": items.get("id"),
                        "COMERCIALFORMAT_ID": CommercialFormat_dict.get("id")
                        if isinstance(CommercialFormat_dict, dict) 
                        else None,

                        "COMERCIALFORMAT_NAME": CommercialFormat_dict.get("name")
                        if isinstance(CommercialFormat_dict, dict) 
                        else None,

                        "COMERCIALFORMAT_INITIALS": CommercialFormat_dict.get("initials")
                        if isinstance(CommercialFormat_dict, dict)
                        else None,
                    }
                    rows.append(row)
...
def loadBigquery(df, table_id):
    ...
    try:
        ...
        schema = [
            ...
            bigquery.SchemaField("COMERCIALFORMAT_ID", "INT64"),
            bigquery.SchemaField("COMERCIALFORMAT_NAME", "STRING"),
            bigquery.SchemaField("COMERCIALFORMAT_INITIALS", "STRING"),
        ]
...
def main(request):
    ...
    table_id = os.environ.get('TABLE_ID_PROGRAMMING_ITEMS_COMERCIAL_FORMAT')
