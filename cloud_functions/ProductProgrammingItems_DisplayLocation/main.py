        ...
        for product in products:
            ...
            for programming in programming_list:
                ...
                for items in items_list:
                    displayLocation_dict = items.get("displayLocation")   
                    row = {
                        "PRODUCT_ID": product_id,
                        "PROGRAMMING_ID": programming.get("id"),
                        "ITEMS_ID": items.get("id"),
                        "DISPLAYLOCATION_ID": displayLocation_dict.get("id")
                        if isinstance(displayLocation_dict, dict) 
                        else None,

                        "DISPLAYLOCATION_NAME": displayLocation_dict.get("name")
                        if isinstance(displayLocation_dict, dict) 
                        else None,

                        "DISPLAYLOCATION_INITIALS": displayLocation_dict.get("initials")
                        if isinstance(displayLocation_dict, dict) 
                        else None,
                    }
                    rows.append(row)
...
def loadBigquery(df, table_id):
    ...
    try:
        ...
        schema = [
            ...
            bigquery.SchemaField("DISPLAYLOCATION_ID", "INT64"),
            bigquery.SchemaField("DISPLAYLOCATION_NAME", "STRING"),
            bigquery.SchemaField("DISPLAYLOCATION_INITIALS", "STRING"),
        ]
...
def main(request):
    ...
    table_id = os.environ.get('TABLE_ID_PROGRAMMING_ITEMS_DISPLAY_LOCATION')
